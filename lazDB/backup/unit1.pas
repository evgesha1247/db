...
procedure TForm1.FormCreate(Sender: TObject);
  begin
    sql := ' select products.idWriter,publishers.idProduct,publishers.idBookBinding ,publishers.idPublisHers ,  publishers.id, writers.writers_name,writers.patronymic,writers.surname,products.products_name, publishingHouse.publisherName ,bookBinding.bookBinding_name ,publishers.circulation from publishers left join products on publishers.idProduct = products.id left join bookBinding on publishers.idBookBinding = bookBinding.id left join writers on  products.idWriter = writers.id  left join publishingHouse on publishers.idPublisHers = publishingHouse.id ';
...
begin
    ...
    SQLQuery.SQL.Add(SQL);
     if Column.FieldName = 'id' then begin
       SQLQuery.SQL.Add('ORDER BY publishers.id');
     end
     else begin
        SQLQuery.SQL.Add('ORDER BY ' + Column.FieldName);
     end;
...
begin
   if EditSearch.text = '' then begin
      ...
      SQLQuery.Active := false;
      SQLQuery.SQL.Add(sql);
      SQLQuery.SQL.Add('where  writers.surname like ''%' + EditSearch.text + '%'';');
      SQLQuery.Active := True;
    end;
end;
